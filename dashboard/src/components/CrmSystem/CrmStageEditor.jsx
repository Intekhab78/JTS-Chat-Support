...
import { DEFAULT_CRM_STAGE_CONFIG } from "./CrmUIComponents.jsx";

export default function CrmStageEditor({ open, onClose, onChangeStages, currentStages }) {
    const [stages, setStages] = useState(() => {
        // If the parent passes down the current persisted stages, use those; otherwise fall back to defaults
        if (Array.isArray(currentStages) && currentStages.length > 0) {
            return currentStages.map(s => ({ key: s.key, label: s.label, active: s.active !== false }));
        }
    ...
    const handleSave = () => {
        if (typeof onChangeStages === "function") onChangeStages(stages);
        onClose();
    };
