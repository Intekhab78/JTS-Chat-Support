          ...
          <section className="overflow-hidden rounded-[30px] border border-slate-200/80 bg-white shadow-sm">
            <div className="border-b border-slate-200 bg-[linear-gradient(180deg,#f8fafc_0%,#ffffff_100%)] px-5 py-5 md:px-6">
              <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5 flex-1 min-w-0 max-w-5xl">
                  {[
                    ...
                    <div key={card.label} className={`min-w-0 rounded-2xl border border-slate-200 px-3.5 py-3 overflow-hidden ${card.bg || "bg-slate-50"}`}>
                      <p className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-400 truncate">{card.label}</p>
                      <p className={`mt-1 text-xs sm:text-sm xl:text-base font-black tracking-tight truncate whitespace-nowrap ${card.color}`} title={String(card.value)}>{card.value}</p>
                    </div>
                  ))}
                </div>
              </div>
