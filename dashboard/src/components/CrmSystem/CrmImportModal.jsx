      ...
      <div className="bg-white border border-slate-200 rounded-[28px] max-w-lg w-full p-6 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between shrink-0">
          ...
          <button 
            onClick={() => {
              if (resultSummary) {
                onSuccess(`Imported ${resultSummary.imported} leads!`);
              }
              onClose();
            }}
        ...
        {resultSummary ? (
          <div className="space-y-4 flex flex-col flex-1 overflow-hidden">
            {/* Top Success Badge */}
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800 font-bold flex items-center gap-3">
              <CheckCircle2 size={20} className="text-amber-600 shrink-0" />
              <div>
                <p className="font-black text-slate-900">Import completed with warnings</p>
                <p className="text-[10px] text-slate-500 font-normal mt-0.5">Successfully imported {resultSummary.imported} leads, but skipped {resultSummary.skipped} rows.</p>
              </div>
            </div>

            {/* Skipped Details Log Table */}
            <div className="border border-slate-200 rounded-2xl overflow-hidden flex flex-col flex-1">
              <div className="bg-slate-50 border-b px-4 py-2 text-[9px] font-black uppercase text-slate-400 tracking-wider">
                Skipped Rows Validation Report
              </div>
              <div className="overflow-y-auto flex-1 pr-0.5 [scrollbar-width:thin] max-h-[220px]">
                <table className="w-full text-left text-[11px] font-bold text-slate-600">
                  <thead className="bg-slate-50/50 sticky top-0 border-b text-[8px] font-black uppercase text-slate-400">
                    <tr>
                      <th className="py-2 px-4">Row</th>
                      <th className="py-2 px-4">Lead Name</th>
                      <th className="py-2 px-4">Skip Reason / Error</th>
                    </tr>
                  </thead>
                  <tbody>
                    {resultSummary.skippedDetails.map((log, idx) => (
                      <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                        <td className="py-2 px-4 font-mono text-slate-400">#{log.row}</td>
                        <td className="py-2 px-4 text-slate-800">{log.name}</td>
                        <td className="py-2 px-4 text-rose-600 font-mono text-[10px]">{log.reason}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="pt-2 shrink-0">
              <button
                ...
                className="w-full bg-slate-900 rounded-2xl py-3.5 text-[10px] font-black uppercase text-white hover:bg-slate-800 transition-all"
              >
                Acknowledge & Close
              </button>
            </div>
