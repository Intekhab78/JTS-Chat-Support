...
async function resolvePersonnelRole(requestedRole) {
  ...
  if (["agent", "manager", "user", "sales"].includes(normalizedRole)) {
  ...
  const matchingRole = await Role.findOne({
    isActive: true,
    $expr: {
      $eq: [{ $toLower: "$name" }, normalizedRole]
    }
  }).select("name");

  if (!matchingRole) {
    throw new AppError("Selected role is not active or does not exist", 400);
  }

  return normalizedRole;
}

function buildPersonnelFilter(scope = {}) {
  return {
    ...scope,
...
export const createAgent = asyncHandler(async (req, res) => {
  ...
  const role = await resolvePersonnelRole(req.body.role);
  ...
  const personnelCount = await User.countDocuments(buildPersonnelFilter({ managerId }));
...
export const updateAgent = asyncHandler(async (req, res) => {
  const parentId = req.user.role === "client" ? req.user._id : req.user.managerId;
  const filter = normalizeRole(req.user.role) === "admin"
    ? buildPersonnelFilter({ _id: req.params.id })
    : buildPersonnelFilter({ _id: req.params.id, managerId: parentId });
  ...
  if (role) {
    agent.role = await resolvePersonnelRole(role);
  }
  if (department) {
...
export const getClientDetails = asyncHandler(async (req, res) => {
  ...
  const personnel = await User.find(buildPersonnelFilter({ managerId: client._id }))
    .select("-password")
    .populate("websiteIds", "websiteName domain")
    .sort({ createdAt: -1 });
