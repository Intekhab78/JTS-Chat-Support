...
import { generateInvoicePDF, generateQuotationPDF } from "../services/pdfService.js";
      ...
      try {
        ...
        if (quotationId) {
          ...
          if (quotation && quotation.status !== "accepted") {
            ...
            try {
              const existingInvoice = await Invoice.findOne({ paymentIntentId: pi.id });
              ...
              const invPdf = await generateInvoicePDF({
                ...invoice.toObject(),
                companyName: typeof quotation.websiteId === "object" ? quotation.websiteId.websiteName : undefined
              });
              ...
              const attachmentsInv = invPdf?.filePath ? [{ path: invPdf.filePath, filename: `${invoice.invoiceId}.pdf` }] : [];
              if (customer?.email) {
                await sendEmail({
                  to: customer.email,
