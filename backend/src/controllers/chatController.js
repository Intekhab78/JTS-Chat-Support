...
import { addMessage, closeSession, findOrCreateSession, registerVisitor } from "../services/chatService.js";
...
import { emitSessionUpdate, getSocketServer } from "../sockets/index.js";
...
function isSessionInTrash(session) {
  const legacyTrashAgeMs = 7 * 86400000;
  return !!session?.archivedAt || (
    !session?.archivedAt
    && !session?.restoredAt
    && session?.status === "closed"
    && (Date.now() - new Date(session.closedAt || session.updatedAt).getTime()) > legacyTrashAgeMs
  );
}
...
export async function submitOfflineMessage(req, res) {
  const website = req.website;
  ...
  const session = await ChatSession.findOne({ sessionId, websiteId: website._id })
    .populate("visitorId")
    .populate("websiteId", "websiteName domain managerId");
  ...
  if (session.visitorId) {
    ...
    if (cleanName) session.visitorId.name = cleanName;
    if (cleanEmail) session.visitorId.email = cleanEmail;
    session.visitorId.lastVisitTime = new Date();
    await session.visitorId.save();
  }
  ...
  session.queueAlertSentAt = null;
  session.offlineMessagePending = true;
  session.offlineMessageSubmittedAt = new Date();
  session.offlineTicketGeneratedAt = null;
  ...
  const payload = {
    ...
    sender: "visitor",
    senderName: session.visitorId?.name || "Visitor",
  ...
  if (io) {
    ...
    io.to(`ws_${session.websiteId._id || session.websiteId}`).emit("chat:new-message", payload);
  }
  ...
  await createActivityEvent({
    ...
    type: "comment_added",
    summary: `Offline message received for chat ${session.sessionId}`,
    metadata: { offlineMessagePending: true }
  });

  emitSessionUpdate(await loadRealtimeSession(session._id));

  return res.status(201).json({
    message: "Offline message saved successfully",
    sessionId: session.sessionId
  });
}
...
export async function archiveChatSession(req, res) {
  requirePermission(req.user, PERMISSIONS.CHAT_VIEW);
  const session = await ChatSession.findOne({ sessionId: req.params.sessionId }).populate("websiteId", "managerId");
  if (!session) {
    return res.status(404).json({ message: "Session not found" });
  ...
  if (session.status !== "closed") {
    return res.status(409).json({ message: "Only closed chats can be moved to trash" });
  }

  session.archivedAt = new Date();
  session.archivedBy = req.user._id;
  session.archiveReason = req.body?.reason || "manual_trash";
  session.restoredAt = null;
  session.restoredBy = null;
  await session.save();
  ...
  await createActivityEvent({
    ...
    entityId: session._id,
    type: "archived",
    summary: `Chat ${session.sessionId} was moved to trash`,
    metadata: { archivedAt: session.archivedAt, archiveReason: session.archiveReason }
  });
  await logAuditEvent({
    actor: req.user,
    ...
    entityId: session._id,
    websiteId: sessionWebsiteId,
    metadata: { sessionId: session.sessionId, archiveReason: session.archiveReason },
    ipAddress: req.ip
  });
...
export async function restoreChatSession(req, res) {
  requirePermission(req.user, PERMISSIONS.CHAT_VIEW);
  const session = await ChatSession.findOne({ sessionId: req.params.sessionId }).populate("websiteId", "managerId");
  if (!session) {
    return res.status(404).json({ message: "Session not found" });
  ...
  if (!isSessionInTrash(session)) {
    return res.status(409).json({ message: "Session is not in trash" });
  }
  ...
  await createActivityEvent({
    ...
    entityId: session._id,
    type: "updated",
    summary: `Chat ${session.sessionId} was restored from trash`,
    metadata: { restoredAt: session.restoredAt }
  });
  await logAuditEvent({
    actor: req.user,
    action: "chat.restored",
    entityType: "chat_session",
    entityId: session._id,
    websiteId: sessionWebsiteId,
    metadata: { sessionId: session.sessionId },
...
export async function deleteChatSession(req, res) {
  try {
    ...
    if (!session) return res.status(404).json({ message: "Session not found" });
    if (!await ensureSessionStaffAccess(session, req.user)) {
      return res.status(403).json({ message: "Access denied" });
    }
    if (!isSessionInTrash(session)) {
      return res.status(409).json({ message: "Only chats in trash can be deleted permanently" });
    }
