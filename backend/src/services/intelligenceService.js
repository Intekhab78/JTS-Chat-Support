...
/**
 * Performs heuristic sentiment analysis and chat summarization.
 * In a production environment, this would call an LLM (OpenAI/Gemini).
 */
export async function processChatIntelligence(sessionId) {
  try {
    ...
    const negativeWords = ["bad", "angry", "terrible", "worst", "hate", "slow", "late", "broken", "fail", "not working", "expensive", "cancel", "stuck", "frustrated"];
    const positiveWords = ["good", "great", "awesome", "perfect", "thanks", "thank you", "love", "fast", "easy", "help", "solve", "happy", "excited"];
    ...
    visitorMessages.forEach(m => {
      ...
      negativeWords.forEach(w => { if (text.includes(w)) score -= 0.5; });
      positiveWords.forEach(w => { if (text.includes(w)) score += 0.3; });
    });

    // Normalize score between -1 and 1
    const normalizedScore = Math.max(-1, Math.min(1, score / (visitorMessages.length || 1)));
    let label = "neutral";
    if (normalizedScore > 0.2) label = "positive";
    if (normalizedScore < -0.2) label = "negative";

    // 2. Automated Summarization (High-Fidelity Simulation)
    // In production: const summary = await callLLM(messages);
    let summary = "";
    if (visitorMessages.length > 0) {
      const firstMsg = visitorMessages[0].message?.substring(0, 50);
      const lastMsg = visitorMessages[visitorMessages.length - 1].message?.substring(0, 50);
      summary = `Visitor initiated chat about "${firstMsg}...". Interaction concluded with "${lastMsg}...". Sentiment is ${label}.`;
    }
